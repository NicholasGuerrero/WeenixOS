...
do_brk(void *addr, void **ret)
{
        ...
        /* The 'p_brk' and 'p_start_brk' members of a proc_t struct are initialized by the loader. */
        // vmarea_t *dataBssEnd_vma = vmmap_lookup(curproc->p_vmmap, ADDR_TO_PN(PAGE_ALIGN_DOWN(curproc->p_start_brk))); /* vma that contains the start brk */

        vmarea_t *dataBssEnd_vma = vmmap_lookup(curproc->p_vmmap, ADDR_TO_PN(curproc->p_start_brk));
        uint32_t dynamicRegion_endPage = ADDR_TO_PN((uint32_t)addr - 1);
        if (dynamicRegion_endPage >= dataBssEnd_vma->vma_end){
         /* If the dynamic reagion is beyond the end of the Data+BSS virtual memory area */
                if (vmmap_is_range_empty(curproc->p_vmmap, dataBssEnd_vma->vma_end, dynamicRegion_endPage - dataBssEnd_vma->vma_end + 1)) {
                        /* If the next vmarea is empty then set the dynamic region's p_brk end address. 
                        * The range will be starting at the end of the dataBss region and extend for the npages specific by addr.
                        *  NOTE (+1) since vma_end is exclusive */
                        dataBssEnd_vma->vma_end = dynamicRegion_endPage + 1;
                        *ret = addr;
                        curproc->p_brk = addr;
                        dbg(DBG_PRINT, "(GRADING3A)\n");
                        return 0;
                ...
                /* otherwise the dynamic region is inside ONE virtual memory area and you can just return */
                dataBssEnd_vma->vma_end = dynamicRegion_endPage + 1;
                *ret = addr;
                curproc->p_brk = addr;
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
