...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        vmarea_t *vm = vmmap_lookup(p->p_vmmap, vfn);
        if(NULL == vm){
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return 0;
        }

        int perm_read = perm & PROT_READ;
        int perm_write = perm & PROT_WRITE;
        int perm_exe = perm & PROT_EXEC;
        int vperm_read = vm->vma_prot & PROT_READ;
        int vperm_write = vm->vma_prot & PROT_WRITE;
        int vperm_exe = vm->vma_prot & PROT_EXEC;

        if(perm_read<=vperm_read && perm_write<=vperm_write && perm_exe<=vperm_exe){
                dbg(DBG_PRINT, "(GRADING3A)\n");
                return 1;
        }
        dbg(DBG_PRINT, "(GRADING3D 1)\n");
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        uint32_t addr_off = 0;
        while(len > addr_off){
                const void *target_addr = (const void*)((uint32_t)avaddr+addr_off);
                int ret = addr_perm(p, target_addr, perm);
                if(1!=ret){
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        return 0;
                }
