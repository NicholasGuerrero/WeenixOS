...
sys_read(read_args_t *arg)
{
        ...
        if(0 > copy_from_user(&kern_args, arg, sizeof(kern_args))){
        ...
        int bytes = do_read(kern_args.fd, buf, kern_args.nbytes);
        if(0 > copy_to_user(arg->buf, buf, bytes)){
...
sys_write(write_args_t *arg)
{
        ...
        if(0 > copy_from_user(buf, kern_args.buf, kern_args.nbytes)){
                curthr->kt_errno = EFAULT;
                page_free(buf);
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return -1;
        ...
        int bytes = do_write(kern_args.fd, buf, kern_args.nbytes);
        if (bytes < 0) {
                page_free(buf);
                curthr->kt_errno = -bytes;
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
...
sys_getdents(getdents_args_t *arg)
{
        ...
        int total=0;
        int num_loop= (int)kern_args.count / sizeof(dirent_t);
        for(int i =0; i < num_loop; i++){
                int ret = do_getdent(kern_args.fd, &dirp);
                if(ret < 0){
                        curthr->kt_errno = -ret;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        return -1;
                }
                if(0 > copy_to_user(kern_args.dirp+i, &dirp, (size_t)ret)){
                        curthr->kt_errno = EFAULT;  
                        dbg(DBG_PRINT, "(GRADING3D 1)\n"); 
                        return -1;
                }
