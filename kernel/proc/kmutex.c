...
{
        mtx->km_holder = NULL;             /* no mtx holder yet */
        sched_queue_init(&mtx->km_waitq);  /* Init the km_waitq */
...
kmutex_lock(kmutex_t *mtx)
{
        ...
        /* If another kthread is holding mute then sleep in the km_waitq */
        if (mtx->km_holder != NULL) {
                sched_sleep_on(&(mtx->km_waitq));  /* sleepy sleepy */
        }
        else {
                mtx->km_holder = curthr;  /* holdy holdy */
        }
...
kmutex_lock_cancellable(kmutex_t *mtx)
{
        ...
        if (mtx->km_holder != NULL) {
                /* If thread is holding the mutex, then CANCELLABLE sleep  */
                return sched_cancellable_sleep_on(&(mtx->km_waitq));
        }
        else {
                /* If no thread is holding the mutex, then give mutex to curthr */
                mtx->km_holder = curthr;
                return 0;
        }

        //KASSERT(curthr && (curthr != mtx->km_holder)); /* on return, curthr must not be the mutex (mtx) holder */
        //dbg(DBG_PRINT, "(GRADING1A 6.b)\n");
...
kmutex_unlock(kmutex_t *mtx)
{
        ...
        mtx->km_holder = sched_wakeup_on(&(mtx->km_waitq)); /* next muetx on the mutex km_waitq get to hold the mutex */
