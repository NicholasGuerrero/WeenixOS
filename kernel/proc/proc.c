...
proc_create(char *name)
{
        ...
        //Assign process name, pagedir and status variables
        ...
        //Setting p_pproc and proc_initproc
        ...
        //Initialize lists and insert to proc_list
        ...
        //Insert to parent process children list if this is not Process0
        if(p->p_pid != PID_IDLE){
...
proc_cleanup(int status)
{
        ...
        curthr->kt_state = KT_EXITED;  /* Change thread state */
        curproc->p_state = PROC_DEAD;  /* Change process state */
        curproc->p_status = status;    /* Update exit status*/

        sched_wakeup_on(&(curproc->p_pproc->p_wait));  /* Wake up parent */
        

        if (!list_empty(&curproc->p_children) && curproc->p_pid != PID_IDLE && curproc->p_state!=PROC_DEAD) {
                list_iterate_begin(&(curproc->p_children), proc_t *p, proc_t, p_child_link) {
...
proc_kill(proc_t *p, int status)
{
        ...
        if(p == curproc){
                ...
                list_iterate_begin(&(p->p_threads), kthread_t *kthr, kthread_t, kt_plink){
...
proc_kill_all()
{
        ...
        list_iterate_begin(&_proc_list, proc_t *p, proc_t, p_list_link){
                
                if (PID_IDLE == p->p_pid) {
                        continue;
                }
                if (p->p_pproc->p_pid != PID_IDLE && p!= curproc){
                        proc_kill(p, 0);
                }
...
do_waitpid(pid_t pid, int options, int *status)
{
        ...
        if(list_empty(&curproc->p_children) || (proc_lookup(pid)==NULL && pid !=-1)){
                return -ECHILD;
        }
                ...
                while(1){
                        list_iterate_begin(&curproc->p_children, child_process, proc_t, p_child_link){
                                ...
                                if(child_process->p_state == PROC_DEAD){
                                        ...
                                        if(NULL!= child_process->p_status){
                                                *status = child_process->p_status;
                                        }
                                        list_iterate_begin(&child_process->p_threads, kthread_t *thrd, kthread_t, kt_plink){
                                                kthread_destroy(thrd);
                                        }list_iterate_end();

                                        /*remove child process's list and parent process's list */
                                        if(!list_empty(&child_process->p_list_link) && !list_empty(&child_process->p_child_link)){
                                                list_remove(&child_process->p_list_link);
                                                list_remove(&child_process->p_child_link);

                                                pt_destroy_pagedir(child_process->p_pagedir);
                                                slab_obj_free(proc_allocator, child_process);

                                        }
                                        

                                        KASSERT(NULL != child_process);
                                        KASSERT(-1== pid || child_process->p_pid == pid);
                                        KASSERT(NULL != child_process->p_pagedir);
                                        dbg(DBG_PRINT, "(GRADING1A 2.c)\n");

                                        return child_process->p_pid;
